...
public class ConnectionManager implements BroadcastInterface {
    ...
    public ConnectionManager(
            ...
            DeviceLookupService deviceLookupService, Geocoder geocoder) {
        this.config = config;
    ...
    public void updateDevice(long deviceId, String status, Date time) throws StorageException {
        ...
        if (!status.equals(oldStatus)) {
            ...
            switch (status) {
                ...
                case Device.STATUS_UNKNOWN:
                    ...
                    device.setMotionStreak(false);
                    device.setMotionTime(null);
                    device.setMotionDistance(0);
                    device.setMotionState(false);
                    Position position = storage.getObject(Position.class, new Request(
                            new Columns.All(),
                            new Condition.Equals("deviceId", device.getId()),
                            new Order("fixTime", true, 1)));
                    events.put(new Event(Event.TYPE_DEVICE_STOPPED, position), null);
        ...
        try {
            storage.updateObject(device, new Request(
                    new Columns.Include("status", "lastUpdate", "motionStreak", "motionTime", "motionDistance", "motionState"),
