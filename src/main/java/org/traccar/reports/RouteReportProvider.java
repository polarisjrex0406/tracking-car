...
public class RouteReportProvider {
    ...
    @Inject
    public RouteReportProvider(Config config, ReportUtils reportUtils, Storage storage, Geocoder geocoder) {
        this.config = config;
    ...
    public Collection<Position> getObjects(long userId, Collection<Long> deviceIds, Collection<Long> groupIds,
        ...
        Position startTrip = result.get(0);
        ...
        if (startAddress == null && geocoder != null && config.getBoolean(Keys.GEOCODER_ON_REQUEST)) {
            ...
            startTrip.setAddress(startAddress);
        }
        storage.updateObject(startTrip, new Request(
                        new Columns.Include("address"),
                        new Condition.Equals("id", startTrip.getId())));
        result.set(0, startTrip);
        Position endTrip = result.get(result.size() - 1);
        ...
        if (endAddress == null && geocoder != null && config.getBoolean(Keys.GEOCODER_ON_REQUEST)) {
            ...
            endTrip.setAddress(endAddress);
        }
        
        storage.updateObject(endTrip, new Request(
                        new Columns.Include("address"),
                        new Condition.Equals("id", endTrip.getId())));
        LogAction.logString(String.format("Address:%s", endAddress));
        result.set(result.size() - 1, endTrip);
        return result;
    }
