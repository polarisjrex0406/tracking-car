    ...
    private Collection<SummaryReportItem> calculateDeviceResult(
        ...
        if (first != null && last != null) {
            ...
            if (first.hasAttribute(Position.KEY_HOURS) && last.hasAttribute(Position.KEY_HOURS)) {
                ...
                long tripDuration = last.getFixTime().getTime() - first.getFixTime().getTime();
                if (tripDuration > 0) {
                    result.setAverageSpeed(UnitsConverter.knotsFromMps(result.getDistance() * 1000 / tripDuration));
                }
            }
